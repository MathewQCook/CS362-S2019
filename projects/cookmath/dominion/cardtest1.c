...
int main() {
	...
	//Restarting the game to test direct call of smithyCard function
	initializeGame(2, k, 2, &GameSt);

	smithyCard(&GameSt, 0);
	postCards = numHandCards(&GameSt); //Should now be 7, 3 cards added and 1 card (the smithy card) discarded
	assert(postCards == 7);
	printf("smithyCard function passed.\n");
