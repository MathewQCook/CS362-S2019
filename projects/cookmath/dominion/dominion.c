...
int adventurerCard(int currentPlayer, struct gameState *state){
    ...
    while(hCounter-1>=0){
        state->discard[currentPlayer][state->discardCount[currentPlayer]++]=temphand[hCounter-1]; // discard all cards in play that have been drawn
        hCounter=hCounter-1;
    }
...
//Refactorization of Salvager
int salvagerCard(int currentPlayer, struct gameState *state, char handPos, int choice1){
      //+1 buy
      state->numBuys++;
...
//End of refactorization, beginning of cardEffect
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card )
    ...
    case sea_hag:
        seahagCard(state, currentPlayer);
